...
def create():
    ...
    result = monitorias_collection.insert_one(monitoria)
...
def get(user_id):
    monitorias = []
    for monitoria in monitorias_collection.find({"user_id": user_id}):  # filtrando as tarefas baseado no usuário autenticado
...
def update(monitoria_id):
    ...
    monitorias_collection.update_one({'_id': ObjectId(monitoria_id)}, {'$set': updated})
...
def delete(monitoria_id):
    monitorias_collection.delete_one({'_id': ObjectId(monitoria_id)})
